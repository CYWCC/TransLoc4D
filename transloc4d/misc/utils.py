...
class ModelParams:
    def __init__(self, model_params_path):
        ...
        self.output_dim = params.getint(
            "output_dim", 256
        )  # Size of the final descriptor
        ...
        if "polar" in self.coordinates:
            ...
            self.quantization_step = tuple(
                [float(e) for e in params["quantization_step"].split(",")]
            )
            assert (
                len(self.quantization_step) == 3
            ), f"Expected 3 quantization steps: for sectors (degrees), rings (meters) and z coordinate (meters)"
        ...
        self.input_representation = params.get("input_representation", "R")
        self.self_att = params.getboolean("self_att", False)
        self.add_FTU = params.getboolean("add_FTU", False)  # add_FTU module
        ...
        if self.model == "TransLoc3d":
            ...
            self.conv0_kernel_size = params.getint("conv0_kernel_size", 5)
            self.conv0_stride = params.getint("conv0_stride", 1)
            self.conv1_out_channels = params.getint("conv1_out_channels", 64)
            self.conv1_kernel_size = params.getint("conv1_kernel_size", 2)
            self.conv1_stride = params.getint("conv1_stride", 2)
            self.num_attn_layers = params.getint("num_attn_layers", 6)
            self.global_channels = params.getint("global_channels", 64)
            self.local_channels = params.getint("local_channels", 0)
            if "num_centers" in params:
                self.num_centers = tuple(
                    [int(e) for e in params["num_centers"].split(",")]
                )
            else:
        ...
        for e in param_dict:
            if e == "quantization_step":
                ...
                if self.coordinates == "polar":
                    print(
                        f"quantization_step - sector: {s[0]} [deg] / ring: {s[1]} [m] / z: {s[2]} [m]"
                    )
                else:
...
class TrainingParams:
    ...
    def __init__(self, params_path: str, model_params_path: str, debug: bool = False):
        ...
        assert os.path.exists(params_path), "Cannot find configuration file: {}".format(
            params_path
        )
        assert os.path.exists(
            model_params_path
        ), "Cannot find model-specific configuration file: {}".format(model_params_path)
        ...
        self._check_params()
        self.model_name = model_params_path.split("/")[-1].split(".")[0]
